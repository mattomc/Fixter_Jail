...
namespace Client
{
    public class Client : BaseScript {
        ...
        private readonly Vector3 _sandyPd = new Vector3(1857.76f, 3689.3f, 34.28f); ///New Sandy PD
        private readonly Vector3 _paletoPd = new Vector3(-448.51f, 6012.71f, 31.72f);
        private readonly Vector3 _bolingbrokePrison = new Vector3(1792.49f, 2593.75f, 45.8f);
        private readonly Vector3 _DelPerro = new Vector3(-1635.22f, -1022.84f, 13.5f);
        private readonly Vector3 _LaMesa = new Vector3(851.72f, -1289.96f, 28.26f);
        ...
        private async Task CheckLocation()
        {
            ...
            if (LocalPlayer.Character.Position.DistanceTo(_DelPerro) <= 1f && !_nui)
            {
                Screen.DisplayHelpTextThisFrame("Press ~y~E~w~ to open jail interface.");
                if (Game.IsControlPressed(0, Control.Context))
                {
            ...
            if (LocalPlayer.Character.Position.DistanceTo(_LaMesa) <= 1f && !_nui)
            {
                Screen.DisplayHelpTextThisFrame("Press ~y~E~w~ to open jail interface.");
                if (Game.IsControlPressed(0, Control.Context))
                {
